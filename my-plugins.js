(function() {
    ...
    // Логика макета как в оригинале: добавляем шаблон full_start_new только если опция включена (стандартный порядок: play, online, torrent)
    function incardtemplate() {
        var incardtemplate = localStorage.getItem('ui_enhancer_incardtemplate') === 'true';
        if (incardtemplate) {
            // Шаблон страницы деталей (фильм/сериал) как в оригинале Lampac
    ...
    // Логика больших кнопок как в оригинале: спаны видимы на десктопе, на мобиле только для focus + overflow
    function bigbuttons() {
        ...
        if (bigbuttons) {
            var bigbuttons_style = "\n<style id=\"ui_enhancer_bigbuttons\">\n " +
                // Стандартная логика: спаны inline на всех экранах, но на мобиле скрыты для non-focus (оригинал Lampac)
