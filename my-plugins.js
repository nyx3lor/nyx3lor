(function() {
    ...
    Lampa.Lang.add({
        ...
        interface_custom_showtv: {
            ...
            be: "Паказваць мітку TV",
            zh: "显示 TV 徽章",
            pt: "Mostrar badge TV",
            bg: "Показвай TV етикет",
            he: "הצג תג TV",
            cs: "Zobrazit TV štítek"
        },
        interface_custom_tvcaption: {
            ru: "СЕРИАЛ",       
            en: "SERIES",   
            uk: "СЕРІАЛ",    
            be: "СЕРЫЯЛ",     
    ...
    var interface_svg = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 3h18v18H3V3zm2 2v14h14V5H5z" fill="currentColor"/></svg>';
    ...
    // Функция инициализации (упрощена без тем)
    function applyTheme() {
        // Удаляем предыдущие стили
        $('#interface_custom_theme').remove();
    ...
    function removeFromSettingsMenu() {
        // Скрываем всё, что может конфликтовать
        ...
        // Устанавливаем дефолтные значения интерфейса
    ...
    // Дополнительные Шаблоны, не меняющиеся от стилей    
    function forall() {
        ...
        // Стили (без темных/светлых)
        var forall_style = "\n<style id=\"interface_custom_forall\">\n " +
            // По центру в мобилке
            ...
            ".full-start__rate \n{margin: 0 0.25em; border-radius: 0.25em;\n padding: 0.3em;\n background-color: rgba(0, 0, 0, 0.3);}\n" +
    ...
    function incardtemplate() {
        var incardtemplate = Lampa.Storage.get('interface_custom_incardtemplate') === 'true';
        if (incardtemplate) {
    ...
    function showtv() {
        var tv_caption = Lampa.Lang.translate('interface_custom_tvcaption');
        var showtv = Lampa.Storage.get('interface_custom_showtv') === 'true';
        $('#interface_custom_showtv').remove(); 
        if (!showtv) {
            var showtv_style = "\n<style id=\"interface_custom_showtv\">\n " +
                ".card__type  {\n  display: none;\n}\n " +
                ".card--tv .card__type {\n  display: none;\n}\n" +
            "</style>\n";
            $('body').append(showtv_style);
        } else {
            var showtv_style = "\n<style id=\"interface_custom_showtv\">\n " +
                ".card__type::after {\n  content: '" + tv_caption + "';\n  display: block;\n}\n" +
    ...
    function bigbuttons() {
        var bigbuttons = Lampa.Storage.get('interface_custom_bigbuttons') === 'true';
        $('#interface_custom_bigbuttons').remove();
        if (bigbuttons) {
            var bigbuttons_style = "\n<style id=\"interface_custom_bigbuttons\">\n " +
    ...
    function startPlugin() {
        ...
        if (Lampa.Storage.get('interface_custom_showtv') === null) {
            Lampa.Storage.set('interface_custom_showtv', 'true');
        }
        if (Lampa.Storage.get('interface_custom_incardtemplate') === null) {
            Lampa.Storage.set('interface_custom_incardtemplate', 'false');
        }   
        if (Lampa.Storage.get('interface_custom_bigbuttons') === null) {
            Lampa.Storage.set('interface_custom_bigbuttons', 'false');
        }
        // Меню        
